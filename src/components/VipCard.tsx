...
export function VipCard({ level, dailyIncome, totalRevenue, price, validity, maxPurchase }: VipCardProps) {
    ...
    const handlePurchase = async () => {
        ...
        // VALIDATION 2: Check maximum purchase limit
        const sameLevelPurchases = userData.planHistory?.filter((p: any) => p.level === level) || [];
        if (sameLevelPurchases.length >= maxPurchase) {
            showToast(`এই প্ল্যান সর্বোচ্চ ${maxPurchase} বার কেনা যায়!`, "error");
            setShowConfirm(false);
            return;
        }
        ...
        try {
            ...
            showToast(`VIP ${level} সফলভাবে ক্রয় হয়েছে!`, "success");
            ...
            <div className="bg-card rounded-xl border border-border overflow-hidden shadow-sm transition-all duration-300 hover:shadow-[0_0_20px_rgba(132,204,22,0.2)] hover:border-primary/50 hover:-translate-y-1">
                ...
                <div className="p-4 space-y-3">
                    <div className="grid grid-cols-2 gap-y-2 text-sm">
                        ...
                        <div className="text-muted-foreground">সর্বোচ্চ পারচেস</div>
                        <div className="text-right font-medium">{maxPurchase}</div>
