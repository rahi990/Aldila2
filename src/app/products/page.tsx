...
export default function TasksPage() {
    ...
    const checkCompletions = (savedCompletions: TaskCompletion[]) => {
        const now = Date.now();
        let updated = false;

        const updatedCompletions = savedCompletions.map(completion => {
            if (!completion.completed && now - completion.startTime >= 120000) {
                updated = true;
                return { ...completion, completed: true, completedAt: now };
            }
            return completion;
        });
    ...
    const handleGoClick = (task: Task) => {
        ...
        if (!existing) {
            ...
            const currentIncome = planIncome[selectedPlan.planId] || 0;
            const newIncome = { ...planIncome, [selectedPlan.planId]: currentIncome + task.reward };
            setPlanIncome(newIncome);
            localStorage.setItem("plan_income", JSON.stringify(newIncome));
